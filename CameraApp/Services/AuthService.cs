    ...
    public class AuthService : IAuthService
    {
        ...
        public async Task<bool> EnsureValidTokenAsync()
        {
            try
            {
                ...
                if (!_currentToken.IsExpired && !_currentToken.IsExpiringSoon)
                {
                    return true;
                }
                ...
                {
                    _logger.LogInformation("Token expirando em breve ({TimeUntilExpiration}), renovando preventivamente", 
                        _currentToken.TimeUntilExpiration);
                }
                
                var refreshToken = _currentToken.RefreshToken;
                if (string.IsNullOrEmpty(refreshToken))
                {
                    _logger.LogError("Refresh token não disponível para renovação automática");
                    await LogoutAsync(); // Força logout se não pode renovar
                    return false;
                }
